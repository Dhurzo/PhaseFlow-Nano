...
4. Updates `outputs/phase-X/.phase` to `in_progress` (already should be; ensures consistency)
...
2. Keep the state as `in_progress` (not a separate PAUSED state — `in_progress` + `remaining-tasks.md` signals pause).
3. ⚠️ Do NOT edit `plan.md`. The phase table is a derived view — regenerated by `phaseflow-doctor --fix`.
